...
class TestRunMergeInDir(unittest.TestCase):
    ...
    def test_reconcile_only_bracketifies_unbracketed_sources(self):
        """
        If a pre-existing merged folder still has unbracketed old terms
        (e.g. 'K-means' without '[K-means]'), reconcile-only should still
        bracketify and replace them with the canonical merged term.
        """
        with tempfile.TemporaryDirectory() as d:
            folder = Path(d)
            jobs = {
                "0": {
                    ...
                    "start": "",
                    "end": "",
                    "Description": "Used K-means in the workflow.",
                    "skillIDs": ["k-means-clustering"],
                }
            }
            skills = {
            ...
            desc = updated["0"]["Description"]
            self.assertIn("[K-means clustering]", desc)
            self.assertNotIn("K-means", desc.replace("[K-means clustering]", ""))
