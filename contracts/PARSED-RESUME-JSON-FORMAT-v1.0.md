# Parsed Resume Object — File/Directory Format (JSON)
...
parsed_resumes/
  <id>/
    ...
    resume.html            # optional — parser output (formatted HTML)
    resume_template.html   # optional — parser output (HTML template)
...
- **resume.html**: Optional. Parser output—formatted HTML version of the resume.
- **resume_template.html**: Optional. Parser output—HTML template.
...
Portfolio: https://sbecker11.github.io/resume-consumer
```
...
```json
{
  "websites": [
    ...
    { "label": "Portfolio", "url": "https://sbecker11.github.io/resume-consumer" }
  ]
}
```
...
The app reads only from **resume-consumer's** `parsed_resumes/` directory (project root). To use a folder you created elsewhere (e.g. `~/workspace-resume/parsed-resumes/parsed-resume-1/`):

1. **Copy or symlink** that folder into resume-consumer's `parsed_resumes/` under the same id:
   - From repo root:  
...
- **Default resume**: Jobs/skills come from `static_content/jobs/jobs.json` and `static_content/skills/skills.json` (API path `/api/resumes/default/data`). No directory under `parsed_resumes/` is required.
...
```js
import { reinitializeResumeSystem } from '@/modules/resume/resumeReinitializer'
...
See the resume reinitializer module for registration of Timeline, CardsController, and resume-list callbacks.
...
- **`currentResumeId: null`** — Loads the **default resume** from `static_content/jobs/jobs.json` and `static_content/skills/skills.json` (API: `/api/resumes/default/data`).
...
1. **Parser output** (e.g. from `resume-to-json` in workspace-resume/resume-parser): writes a **flattened** output folder (no subfolders) with `jobs.json`, `skills.json`, `categories.json`, and a **copy of the original resume** (DOCX/PDF) under its original filename (e.g. `~/workspace-resume/parsed-resumes/parsed-resume-1/`).
2. **Symlink into resume-consumer** (from resume-consumer repo root):  
   `ln -s ~/workspace-resume/parsed-resumes/parsed-resume-1 ./parsed_resumes/parsed-resume-1`
...
- **Reinit path**: Load jobs by id → Timeline reinit → CardsController reinit → resume list reinit.
...
To run the parse-resume pipeline from resume-consumer given a `resume.docx` path:

- **Parser**: `resume-to-json` (in a separate repo, e.g. `RESUME_PARSER_PATH` or `~/workspace-resume/resume-parser`).
- **Invocation**: `resume-to-json <path-to-resume.docx> -o <output-dir>`.
...
**Script:** A parse-resume script (e.g. npm script `parse-resume`) invokes the parser subprocess (`resume-to-json ...`).
...
# Output to resume-consumer's parsed_resumes/<id> (then symlink or use as currentResumeId)
...
Set `RESUME_PARSER_PATH` if the parser repo is not at `../workspace-resume/resume-parser` relative to resume-consumer. The script uses the parser's `venv/bin/python` when present.
...
**Output:** The script must write the HTML to `<resumeFolderPath>/rendered.html`. **resume-consumer** creates this file when the user clicks Render (the script is invoked by resume-consumer's server). resume-consumer then serves it at `GET /api/resumes/:id/rendered`.

If `RESUME_HTML_RENDERER_SCRIPT` is not set, the Render button returns 501 with an explanatory message.

---

## resume-consumer manual updates

**resume-consumer** can write and update the parsed resume JSON files via its API (Resume Details Editor and related endpoints). Users can edit `meta.json`, `other-sections.json`, `categories.json`, and job/skill linkages in `jobs.json` and `skills.json`. These updates persist to disk in the parsed resume folder.
...
| Version | Date       | Change |
|---------|------------|--------|
| 1.0     | 2025-03-08 | Initial version; moved from schemas/ to contracts/ with versioned filename. |
