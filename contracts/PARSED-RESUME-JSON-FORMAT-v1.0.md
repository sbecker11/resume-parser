...
**Output:** The script must write the HTML to `<resumeFolderPath>/rendered.html`. **resume-flock** creates this file when the user clicks Render (the script is invoked by resume-flock's server). resume-flock then serves it at `GET /api/resumes/:id/rendered`.

If `RESUME_HTML_RENDERER_SCRIPT` is not set, the Render button returns 501 with an explanatory message.

---

## Resume-flock manual updates

**resume-flock** can write and update the parsed resume JSON files via its API (Resume Details Editor and related endpoints). Users can edit `meta.json`, `other-sections.json`, `categories.json`, and job/skill linkages in `jobs.json` and `skills.json`. These updates persist to disk in the parsed resume folder.
