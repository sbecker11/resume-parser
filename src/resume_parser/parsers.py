...
def _looks_like_education_job(job: dict[str, Any]) -> bool:
    ...
    if is_non_degree_role(role):
        return False
    # Strictly gate education by legitimate degree in the role.
    # Do not require school-keywords in the institution name because
    # many resumes use abbreviations (e.g., MIT, BYU).
    return True
    ...
    for start in starts:
        ...
        for chunk in chunks:
            ...
            if not school:
                ...
                #   "PhD, ... | MIT 1997"
                #   "BS, ...  | BYU 1987"
                # In these cases, the institution is often an abbreviation
                # that does not match our school-keyword regex.
                parsed_any = False
                for i, ln in enumerate(chunk):
                    ln_s = str(ln)
                    if not has_legitimate_degree(ln_s):
                        continue
                    if "|" not in ln_s:
                        continue
                    if is_non_degree_role(ln_s):
                        continue

                    left, right = ln_s.split("|", 1)
                    degree = left.strip()
                    if not has_legitimate_degree(degree) or is_non_degree_role(degree):
                        continue
                    ...
                    for ln2 in chunk[i + 1 :]:
                        ...
                        if not ln2_s:
                            continue
                        if has_legitimate_degree(ln2_s):
                            break
                    ...
                    # Extract years from this specific row, not the whole chunk.
                ...
                if not parsed_any:
                    continue
                # We handled this chunk via table-like parsing.
                continue
